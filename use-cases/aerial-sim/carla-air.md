...
> **最佳自救＝一條原則，不是逐條補丁。** §架構 那句「繼承了兩邊的所有侷限」其實是五條，而救它們是**同一招**：**解耦——把每條會咬人的侷限推給一個外部 best-in-class 工具，CARLA-Air 只留它無法被取代的那一格（單 tick 空地共模 + 城市語意世界）。** 下方 A2 就是這招的第一個實例（讓 CARLA-Air 管渲染器、RotorPy 管物理）；把**同一個動作**套到每條侷限：

| 繼承的侷限 | 來自 | 自救＝推給誰 | 性質 |
|---|---|---|---|
| 剛體動力學 | AirSim | RotorPy 等外部 FDM（**下方 A2**） | ✅ 乾淨解耦、最高 ROI |
| 遊戲引擎外觀 | CARLA/UE | 外部生成增強（上方 [Carla2Real-2026](./carla2real-2026.md)） | ⚠ buildable-but-novel |
| 版本鎖死 | AirSim 封存 | 解耦越深、鎖越不咬人（**下方 B**） | ✅ A2 的副產品 |
