<!-- ontology-5axis output=action-seq injection=sim-in-loop control=action|trajectory|image-prompt temporal=streaming domain=robotics -->
...
| Control | `image-prompt`（FPV）+ implicit gate-traj + IMU | GPS + obstacle map | full state + waypoint list | image only | image (model-based RL) |
...
| Line | 對應位置 |
|---|---|
| pixel-WM (Sora/Veo/Cosmos-Predict) | Swift 完全不出 pixel；但 Cosmos-style WM 反過來可當 Swift 的「sim renderer」候選 — 取代 Flightmare 的 GPU 渲染，給 policy 更真實的 motion blur / lighting。bottleneck 是 latency（Cosmos 不是 streaming sim）。 |
| latent-WM (DreamerV4 / V-JEPA-2) | SkyDreamer (arXiv 2510.14783, 2025) 已做 — model-based RL 用 world model latent 出 control。比 Swift sample-efficient 但 deploy 穩定性待證。 |
| diff-sim (Brax/MJX/Genesis) | Swift 沒用 differentiable sim，PPO model-free。後續「Learning on the Fly: Rapid Policy Adaptation via Differentiable Simulation」(2025) 已示範用 diff-sim 在線適配，是 Swift 的自然延伸。 |
| neural-surrogate (FNO / GraphCast) | aerial domain CFD 還沒 close 進 control loop（rotor airflow 是極端 multi-scale）。短期內不接。 |
