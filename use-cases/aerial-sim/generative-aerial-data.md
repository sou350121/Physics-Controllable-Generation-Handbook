...
> 本篇解構把 handbook 的核心論點套到 aerial：**generation 擁有 appearance，物理擁有 dynamics。** 它之所以贏得一個 anchor 名額，是因為 aerial 正是這個 contract 最吃重的地方 —— 一個信任被臆造出來的 dynamics 的 policy 不會只是軟性失敗，它會墜機。這個切法本身就是教訓，不是實作細節。
...
最強的單一證據：**一個 3DGS renderer 與一個四旋翼物理模型並排接線、永不交叉，就能產出一個 zero-shot 的真實世界 policy。** 這就是論點本身，已驗證。
...
## 2. 契約：生成 / neural-rendering 能 vs 不能提供什麼

這是進場前要簽的契約。把 generation 當外觀引擎用，它很強；把它當動力學引擎用，它會騙你。

| 能力 | **能提供（generation/neural-rendering）** | **不能提供（需物理模型/感測）** |
|---|---|---|
...
> 契約一句話：**generation 賣給你的是 photons，不是 forces。** 把 forces 外包給它，等於把墜機風險寫進你的 training set。

## 3. 已驗證的證據點 —— 贏的系統都架構化地切開
...
> 看起來最漂亮的影片往往最不符合物理。realism 是一個 appearance 指標；它與 dynamics 正交（略呈負相關）。這正是為什麼 generation 絕不能擁有 contract 中 dynamics 那一半。
