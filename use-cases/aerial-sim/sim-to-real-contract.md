...
這是整份契約裡**最重要的微妙之處**，而且是一條公開的研究張力，不是定論。
...
**同一端的旁證**：**Ferede「One Net to Rule Them All」**（跨平台 DR）直接把這條取捨講白——**0% 隨機化會 sim-to-real 失敗；DR 越寬越魯棒、但越慢**。**MAVEN**（meta-RL）走 in-context 自適應，與 RAPTOR 同陣營。**Song「optimal control vs RL」（SciRob 2023）** 給了上游理由：RL 在真機競速勝過最優控制，正是因為它**能用 DR 把未建模動力學吸收掉**，而不必 commit 到一條模型受限的顯式軌跡。
