...
>
> **為什麼進 embodied-policy-rollout 錨點名單**：本手冊四條 generation 路線裡，latent-WM 這條最反直覺的主張是 ——「策略可以完全不碰真實互動、只在一個學來的世界模型的 latent 夢境裡學會行動」。Dreamer 線是這個主張的標準代表，而且 DayDreamer 把它從模擬器搬到**四台真機**上跑通了。它在 ontology 上乾淨佔住 `temporal=latent-rollout` × `output=action-seq` × `injection=sim-in-loop-train` 這個座標，是 [overview.md](./overview.md) 「Actor-critic on WM」派的錨點。但「想像即決策」這句話藏著一份**承重契約**（§5）—— 那是本篇的智力核心：想像之所以能換掉真實互動，是因為世界模型的 reward / continue head 與短程動力學「必須對」。看懂這份契約，才看得懂 Dreamer 的貢獻邊界與崩潰邊界。
>
> **與姊妹篇的分工**：[../aerial-sim/dream-to-fly.md](../aerial-sim/dream-to-fly.md) 已把 DreamerV3 飛無人機（aerial、HIL、raw-pixel）那一案講透 —— **本篇不重複那個無人機案例**，本篇講通用具身（DayDreamer 的四足 / 雙臂 / 輪式真機）與「WM-as-policy 的承重契約」這個抽象問題。

## 1. 一句話總結

**Dreamer 的核心主張：把一個共用的 latent world model（RSSM）學出來，然後讓 actor 與 critic 純粹在這個世界模型「想像」出來的 latent 軌跡上學行為 —— agent 碰真實環境，只為了把經驗填回 replay buffer 去把世界模型訓得更準。** 行為學習期間（想像視界 **T=16**）梯度從不流經真實 reward、也不流經真實像素，整段 actor-critic 跑在模型的夢裡。DreamerV3 用**一套固定超參**跨 **150+ 任務**全部勝出，並在 Minecraft **從零、無任何人類資料**自己採到鑽石。
...
## 2. 核心機制（RSSM + 想像視界 T=16 + 三個 head）

Dreamer 由**三個網路**構成：(1) **world model**（RSSM：encoder 把觀察壓成 latent；recurrent 部分維持 deterministic 狀態）、(2) **actor**、(3) **critic**。世界模型的 RSSM 把高維觀察壓成 latent 狀態 `s_t = (h_t, z_t)`：`h_t` 是 deterministic recurrent 狀態（攜帶歷史），`z_t` 是 discrete latent（捕捉當下不確定性，posterior 由 encoder 給、prior 由 dynamics 給）。世界模型掛**三個 head**：**reward predictor + continuation(episode-end) predictor + decoder（重建觀察）**。

**承重點先講白**：★**actor-critic 的行為學習期間，從不看真實 reward —— 它最大化的是 reward head 想像出來的回報。因此 reward head 與 continue head 是承重件**，它們錯了，想像回報就錯，策略就學歪。decoder 只是世界模型訓練的輔助目標，policy 不依賴它生成的像素。
...
**規模即性能**：DreamerV3 提供 **6 種尺寸（12M → 400M params，各自單張 A100 即可訓）**，而且**越大越好、且越大需要的資料越少** —— 這跟 model-free 常常 saturate 形成乾淨對照。最戲劇性的存在證明：Minecraft **從零、無任何人類資料**採到鑽石（**100M steps，約 9 天單 GPU**）。

## 3. ⚡ 強 / ❌ 崩

⚡ **強（樣本效率、通用性、真機 DayDreamer）**

- **樣本效率是關鍵使能項，不是錦上添花**：把真實互動攤提成世界模型訓練後，想像 rollout 近乎免費。DayDreamer 的真機數字就靠這個 —— 四足**約 1 小時**從零學會走，是 model-based imagination 直接讓「真機上學連續控制」變得可負擔。每任務對 model-free 的具體加速倍數在原文（標 [UNVERIFIED]，本篇不替論文背書未經查證的數字）。
- **通用性是被證過的**：DreamerV3 **一套固定超參跨 150+ 任務**全勝 —— 把「model-based RL 不通用、得逐域調參」這條先驗直接打破。
- **真機、無模擬器（DayDreamer 的硬核賣點）**：Dreamer 線上**直接在真實世界的四台機器人**上跑 —— 四足（從零、**無重置**、約 1 小時走、約 10 分鐘適應抗推）、雙臂（相機 + 稀疏 reward 的 pick-place，「接近人類水準」）、輪式（視覺導航）。**無重置**這點尤其重要：真機沒有「reset 大法」，Dreamer 的閉迴路在這種非理想條件下仍站得住。
- **規模友善**：12M→400M 六擋，**單張 A100** 即可訓任一擋；越大越好且越省資料。學界可負擔，不需 GPU 農場。
...
- **decoder 偽影不等於 policy 壞**：RSSM decoder 重建常常糊（尤其 UI / 細節），但那是輔助目標；policy 品質看的是 reward 曲線，不是重建清晰度。新手常把重建糊讀成「沒學好」，是誤判。
...
- **vs TD-MPC2** —— 兩者共用 latent dynamics，但 **Dreamer 把 planning 攤提（amortize）進 actor，TD-MPC 每步重新 plan**。取捨：Dreamer **部署便宜**（一次前向出 action）、但對 distribution shift 較脆；TD-MPC **re-plan 對 shift 更穩健**、但 long-horizon planning cost 線性放大。Dreamer 勝在「推理便宜、policy 多次重用」，MPC 勝在「任務多變、policy 不好預訓」（這條取捨與 [overview.md](./overview.md) 兩派分界一致）。
- **vs V-JEPA-2** —— V-JEPA-2 押注 **representation transfer**（看大量 unlabeled video 學表示，再接 action head），latent 是「為表示而生」；Dreamer 的 latent 是「**為 imagination rollout 與 control 而生**」、從第一天就閉迴路。兩者在「offline video → 可控 agent」正面對撞，**差別在 latent 是否一開始就為 dynamics 設計**。
- **vs MuZero / EfficientZero** —— 同屬 latent-WM，但用 **MCTS tree search** 做決策（離散規劃）；Dreamer 用 **stochastic latent + gradient-based policy improvement**。Dreamer 在連續控制（DayDreamer 真機）更自然，tree search 在離散動作 / board game 更強。
- **vs Genie-2** —— Genie 是可互動的**可遊玩** WM，主打人玩；**不在 WM 內做 RL、無 actor-critic**。Dreamer 是 WM 內訓 policy。

> **跨軸註記**：`temporal=latent-rollout` × `injection=sim-in-loop-train` × `output=action-seq` 這個座標的精髓在 **Injection 軸**（本手冊的 USP 軸）—— Dreamer 的世界模型扮演的就是一個「**從資料學出來的、可微的 in-loop simulator**」，actor 的梯度穿過它。這跟 Genesis/MJX 那種「人寫物理方程的可微 sim」是同一個 ontology 格子、不同的物理來源（學來的 vs 寫死的）。
...
**最關鍵的跨路線命題**：**Dreamer 把「latent-WM + 樣本效率」做成了 WM-as-policy 的範本 —— 它證明你可以用廉價的想像 rollout 換掉昂貴的真實互動，但這筆交換的價格寫在 §5 的承重契約裡（reward / continue / 短程動力學必須對 + 策略待在分布內）。**
...
- 與 [../../bridge-to-vla/world-model-as-policy.md](../../bridge-to-vla/world-model-as-policy.md)：Dreamer 是「WM rollout **即決策**」（而非「生成資料訓 policy」）這條橋接線的錨點之一。
- **與 aerial 姊妹篇 [../aerial-sim/dream-to-fly.md](../aerial-sim/dream-to-fly.md)**：那篇是同一 DreamerV3 引擎在**無人機**上的案例（HIL / raw-pixel / 視覺 gap 被旁路的星號）；本篇是同引擎在**通用具身真機**上的案例（DayDreamer / 無模擬器 / 承重契約）。**兩篇姊妹、不重複**：要看「latent-WM 飛無人機」去那篇，要看「latent-WM 的 WM-as-policy 抽象與真機落地」看本篇。
...
- WM-as-policy 橋接：[../../bridge-to-vla/world-model-as-policy.md](../../bridge-to-vla/world-model-as-policy.md)。

**姊妹篇（aerial，DreamerV3 飛無人機 —— 不重複該案例）**
...
| §8.7 | 🟡 | **DayDreamer 每任務對 model-free 的加速倍數未在本篇查證** —— 數字在原文但本篇不替未驗證數字背書 | DayDreamer arXiv [2206.14176](https://arxiv.org/abs/2206.14176)（正文有數字，本篇標 [UNVERIFIED]） | 引用具體加速倍數前回原文 §experiments 核對；勿從二手摘要轉述 |

**[UNVERIFIED] 標記彙總**：(1) §3 / §8.7 DayDreamer 每任務對 model-free 的具體加速倍數（數字在原文，本篇未逐一查證、不背書）；(2) §8.3 imagination horizon 超過多少步開始顯著退化的精確步數界（推論 T=16 是上界附近，論文未在本篇引用範圍內給出精確界）。所有其他數值（imagination horizon **T=16**、三 head = reward/continue/decoder、固定超參 **>150 任務**、6 種尺寸 **12M→400M params 各單 A100**、Minecraft 從零無人類資料 **100M steps ~9 天單 GPU**、free bits **KL clip 1 nat**、DayDreamer 四足 **~1 小時學會走 / ~10 分鐘適應抗推 / 四台真機 / 無重置**）均出自原始來源。
