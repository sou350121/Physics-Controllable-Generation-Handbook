...
> **解構對象**：不是某一個模型，而是 2024–2026 累積出來的「**導演控制堆疊**」（reference / first-last-frame / camera / in-context-edit / structure）—— 加上它在商業量產裡的**真實落點**（廣告正式量產可用 vs 高端電影僅限 previz）。
> **錨定來源**：可控生成綜述（arXiv [2507.16869](https://arxiv.org/html/2507.16869v3)，v3）+ CameraCtrl（arXiv [2404.02101](https://arxiv.org/abs/2404.02101)）+ Runway Aleph（[研究貼文](https://runwayml.com/research/introducing-runway-aleph)，2025-07-25）+ Coca-Cola 2025 假日廣告（[Marketing Dive](https://www.marketingdive.com/news/what-coca-cola-learned-generative-ai/741709/)）+ ILM「Star Wars: Field Guide」（[Decrypt](https://decrypt.co/320224/ilm-makes-star-wars-field-guide-short-film-using-generative-ai)，2025）+ match moving（[Wikipedia](https://en.wikipedia.org/wiki/Match_moving)）。版本宣稱已標日期；快速演進領域，**讀者請以發布日為準**。
> **為什麼進名單**：本倉前面拆的是**引擎**（Sora / AnimateDiff），但創意技術 / VFX 從業者每天面對的不是引擎、是**控制面**——「我能把哪幾根桿子交給生成式？哪幾根還得用傳統管線？」這篇把那組桿子列成一張表，並回答 VFX 圈最痛的一句：**為什麼 gen-video 卡在 previz 進不了 final frame**。答案不在「畫質不夠」，在 axis-3 控制的**類型**——導演要的是**學習式/相對**控制（已成熟），VFX 整合要的是 **metric 控制**（還沒有）。

---

## §1 · 一句話總結（學習式/相對控制已成熟；VFX 要的 metric 控制還沒有）
...
> ★**核心契約**：媒體裡**外觀是產品、物理只需可信不需正確**——這正是本倉 `injection=data-only` 在 media 領域容忍度最高的原因。但**一旦生成像素要與真實素材合成**，幾何與相機就從「可信近似」升級成 **metric 量測約束**，data-only 路線當場破功。導演要的控制（reference / keyframe / camera / in-context-edit）**已成熟**；VFX 要的 metric 控制**還沒有** → 結論：**停在 previz / augmentation，不是 final frame**。

**三條可證偽判斷**（standalone 短內容 / previz 已綠燈；final frame 合成仍紅燈）：
1. 學習式控制堆疊在「**不需與真實素材像素級合成**」的場景已可正式量產（社群短片、廣告、previz、storyboard）。
2. final frame VFX 整合的瓶頸**不是解析度/時長/畫質**，是 axis-3 控制缺 metric 相機解——這是**幾何問題不是視覺問題**。
3. 在 metric 相機/深度求解被生成式原生產出（而非事後 match-move 補）之前，gen-video 在高端管線的角色是 **previz / augmentation，不是交付**。

---

## §2 · 控制堆疊（導演手上的五根桿子）

可控生成綜述（arXiv [2507.16869](https://arxiv.org/html/2507.16869v3)）把條件分成 **7 類**：**Structure**（pose / depth / sketch / bbox）· **Identity** · **Image**（reference）· **Temporal**（flow / trajectory / camera / motion）· **Audio** · **Other** · **Universal**——這與本倉 5 軸的 `control=` 列舉**直接呼應**（structure→layout/pose、image→`image-init`、temporal→`camera`/`trajectory`）。下面把綜述的分類**翻譯成導演實際在用的五根操作桿**：

| 桿子 | 代表能力（版本/日期） | 綜述類別 | 本倉 axis-3 | 控制性質 | 量產槓桿點 |
|---|---|---|---|---|---|
...
| **② first-last-frame（關鍵幀）** | Kling **FLF2V**；Pika **Pikaframes**（1–10s）；Veo **3.1** image-to-image transition | Temporal（motion 端點） | `image-init`×2 | **learned**（端點給定、中間插值由統計補） | **最適合量產的桿**——控制「in 與 out」，接得上 storyboard |
...
**讀法**：①②⑤ 是「**生成內容的內部控制**」（角色/端點/構圖）；③④ 是「**朝向 VFX 管線的兩根桿**」——③ 想控相機、④ 想編輯已有素材。但**③ 的相機是學習式的、④ 的編輯不輸出 metric 幾何**——這正是 §4 那道牆的兩個入口。
...
| 落點 | **正式量產可用**（真的當成品投放） | **僅限 previz**（生物/概念測試，非交付鏡頭） |
...
**收斂結論**：
- 廣告敢上正式量產可用，是因為**很多鏡頭不需要與真實素材像素級合成**（純生成片段拼接即可），且觀眾對廣告的「合成感」容忍度較高——但**人臉**例外（Coca-Cola 兩年都栽在這）。
- 電影守在僅限 previz，是因為高端 VFX 的命脈是**和實拍 plate 無縫合成**——而那需要 §4 的 metric 相機解，**生成式現在給不出**。
- 注：**Sora 2**（OpenAI 已宣布下線：app **2026-04-26**、API **2026-09-24**）在此只當「**一個世代證明了什麼可能**」的能力標記——**不是前向量產目標**。它證明了 cameo/一致性的上限，但工程依賴不該押在一個即將退役的 API 上。
...
**這是幾何問題，不是畫質問題**。哪怕生成畫質再上一個量級，只要相機控制停在「學習式/相對」、不輸出校準 metric 解，gen-video 就跨不過 final frame 的合成門檻。④ in-context edit（Aleph）是最接近的嘗試——它**對已有素材操作**，理論上更靠近管線；但它生「不同相機角度」時同樣**不導出 metric 相機解**，仍是學習式重繪。
...
| Axis | 值 | 理由 |
|---|---|---|
| **Output** | `pixel-video` | 五根桿子最終都 decode 成 RGB 幀；無 3D/顯式幾何輸出 |
| **Injection** | `data-only` | 控制堆疊全靠資料統計——reference 嵌入、首尾幀插值、Plücker→attention、結構條件，**無物理 loss / 無 sim / 無硬約束**（media 領域物理容忍度最高的直接後果） |
...
- **連 [overview.md](./overview.md)**：overview 列的願望清單（ControlNet-video 在電影工程的整合）——本篇給出**為什麼整合卡在 previz** 的根因（metric 牆），是那條願望清單的「下半場」。
- **連引擎章**：[sora.md](../../foundations/video-world-models/sora.md)（`data-only` 參考基準，含 Sora-2 cameo 一致性上限）· [animatediff.md](../../foundations/controllability-mechanisms/animatediff.md)（"freeze base + 學 temporal sidecar"，⑤structure 桿的血統）。**本篇不重拆引擎**，只談架在引擎上的控制面。
- **連 [ontology.md](../../cheat-sheet/ontology.md)**：綜述的 7 類條件 ↔ 本倉 5 軸 `control=` 列舉的對照（structure→layout/pose、image→image-init、temporal→camera/trajectory），是 ontology「多模態可控性是 2026 主戰場」論點的 media 領域實例。

---

## §7 · 參考

- 可控生成綜述（7 類條件分類）：arXiv [2507.16869](https://arxiv.org/html/2507.16869v3)（v3）
...
- Runway **Gen-4**（single-reference 角色一致、VFX 定位）·Sora 2 **cameo**·Kling **FLF2V**·Pika **Pikaframes**·Veo **3.1**·Wan **VACE**（產品/官方頁面，能力宣稱按發布日，**部分 UNVERIFIED**）
...
- **ILM「Star Wars: Field Guide」**（2 分鐘、首個 text-to-video 生物測試、僅限 previz 自評，2025）：[decrypt.co/320224](https://decrypt.co/320224/ilm-makes-star-wars-field-guide-short-film-using-generative-ai)
...
> 來源：(a) 官方/產品自承 limitation、(b) 公開案例可觀察破綻、(c) 概念解讀陷阱。嚴重度：H=阻止用於該用途 / M=工程可繞 / L=外觀層。**所有「角色一致/相機可控」能力宣稱皆產品方自述，未經獨立驗證者標 UNVERIFIED。版本宣稱按發布日，快速演進、易過期。**
...
| §8.3 | ILM 自評 | 「**not a finished product … early-stage**」——高端管線只把它當 ideation 加速 | **H** | 定位為 previz/storyboard 工具；不承諾交付鏡頭；產出進 review 而非 master |
| §8.4 | **Sora 2 下線**（app 2026-04-26 / API 2026-09-24） | 當「能力標記」可以，當**前向量產依賴**會踩空 | **H** | 工程依賴分散到已上線的引擎（Veo / Kling / Wan / Runway）；別綁單一退役 API |
...
| §8.6 | **能力宣稱 = 產品方自述** | Gen-4「無限角色一致」、Sora 2 cameo「最佳 2–4s/≤2 角色」、Kling FLF2V 等**皆官方宣稱**（未經獨立評測 **UNVERIFIED**） | M（**UNVERIFIED**） | 上量產前自行壓測一致性/時長極限；不照單全收行銷數字 |
| §8.7 | **audit Check 9c**（generalist 白名單） | ontology 規定 `domain=generalist` 僅 Sora/Veo/Cosmos 可標；本篇為 **use-case 控制面綜述**（橫跨多 generalist 引擎），非單一模型錨點 → 故沿用 header 指定值並在此聲明 | L | 視作 use-case 級 generalist（聚合多基礎引擎的控制面），非模型級宣稱；若 audit 嚴格化可改標 `media` |
...
| §8.10 | in-context edit（Aleph）仍不導出幾何 | 「生不同相機角度」是**學習式重繪**、非 metric 視角合成——靠近管線但**未過 metric 牆** | M | 當高階 v2v 編輯/概念探索用；不當作可合成 final plate 的多視角幾何源 |
