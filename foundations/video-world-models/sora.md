...
Sora 1.0（OpenAI tech report "Video generation models as world simulators", 2024-02-15）是 video-gen 的 GPT-moment：第一個對外公開的、能 clip-parallel 出 ~60s、1080p 的 text-to-video。它的賭注很單一 — **scale + DiT + 變長 spacetime patches**，物理規律完全靠資料 implicit 學，不掛任何 PDE / contact / 守恆 loss。它的價值對本 handbook 不在「物理感真的解了」(沒有)，而在於它**把 data-only 路線推到一個極限**：用足夠大的 latent diffusion transformer + 變形/長度不對齊的訓練 corpus，能撐出什麼樣的 emergent physics — 以及哪些 physics 它即使在巔峰時也仍然崩。Sora-2（2025-09-30 發布）加 audio + 自家的 "referee model" 物理檢核回路；OpenAI 已宣布 Sora app 於 2026-04-26 關閉、API 2026-09-24 退役，整個品牌處於 sunset 狀態 — 但作為公開最完整的 data-only 案例研究，價值不會過期。
...
```
output     = pixel-video
injection  = data-only       ← Sora-2 加 referee 但仍非顯式 loss
control    = text | image-init
temporal   = clip-parallel            ← 不是 AR
domain     = generalist
```

同軸對手（皆 pixel-video / data-only / clip-parallel / generalist）：
...
關鍵分歧：Cosmos 公開權重給下游接 robotics conditioning，Sora 完全閉源；Veo 走 Google 線（image-init 與 inpaint 更強）；Hunyuan/Wan 開源權重，社區實測物理 case 與 Sora 已可一戰。
...
對本 handbook 的 USP：Sora 是 **Axis-2 data-only 的 reference point** — 任何宣稱「我加了 aux-loss / sim-in-loop / hard-constraint」的方法，benchmark 都該回答「比 Sora-class scale 的純資料路線好多少」。
...
|---|---|---|---|---|
| 8.1 | OpenAI report (官方) | 玻璃杯落地不碎、像橡膠彈 | high | 無 — data-only 路線在剛體破裂 / brittle fracture 上沒有 inductive bias |
...
| 8.4 | PhyWorld (arxiv 2411.02385) | OOD 全面失敗；case-based 而非 principled | structural | scaling 不解；需引入 inductive bias（aux-loss / sim-in-loop） |
