<!-- ontology-5axis output=pixel-video|latent injection=implicit-from-data|sim-in-loop control=text|image-prompt|trajectory|action temporal=hierarchical domain=generalist|robotics|driving -->
...
Cosmos 不是「另一個 Sora」— 是 NVIDIA 為 **physical AI 開發者** 鋪的 open-weight pipeline：把 video FM 的 pre-training cost (10K H100 × 3 個月、~20M hours of video → ~10^8 clips) 一次燒掉，下游 robotics / driving 團隊用幾百~幾千 GPU-hour 做 post-train 就能拿到一個帶有 prompt + image + 軌跡 conditioning 的 world simulator。Prior gap 很具體：Sora / Veo 是 closed weight 且不收 action / trajectory；GAIA-1/2 是 Wayve 內部 driving-only；V-JEPA-2 是 latent representation 不出 pixel。Cosmos 是第一個 **(a) open-weight, (b) generalist pre-train, (c) 顯式支援 image2world / video2world / 多模 control, (d) 配套 Reason-VLM + Tokenizer + Transfer multi-controlnet** 的 stack。它的賭注是：pixel-video FM 的 implicit physics + 下游 sim-in-loop reward 比 hard PDE 路線更 scalable。
...
| Injection | implicit-from-data + sim-in-loop（post-train 可掛 Genesis / Isaac） | implicit | implicit | implicit + structured cond | implicit |
...
| Temporal | joint-rollout（diffusion）/ AR；hierarchical（Reason + Predict） | joint-rollout | joint-rollout | joint-rollout, multi-view consistent | latent-rollout |
...
- **Object permanence / 3D consistency**: 鏡頭環繞或長平移時物件 morph，這是 implicit-from-data 路線的通病；GAIA-2 多 camera consistency 設計就是針對這點。
...
- **× diff-sim (Genesis / MJX)**: Cosmos 在 axis 2 = `implicit-from-data`，diff-sim = `hard-PDE / sim-in-loop`。組合方式：Genesis 出 ground-truth physics rollout（粗糙渲染）→ Cosmos-Transfer 2.5 用 depth+seg 當 control 貼 photoreal 皮。這是 **「physics 由 sim 保證、視覺由 FM 補」** 的標準 production pattern，Wayve / NVIDIA Isaac 均採。
- **× neural surrogate (GraphCast / FNO)**: 不直接對接 — surrogate 是 field output，Cosmos 是 pixel；只在 scientific viz 場景組合（surrogate 算流場 → renderer → Cosmos refine 視覺）。
...
- **× latent-WM (DreamerV4, V-JEPA-2)**: Cosmos 的 DV tokenizer 本身就提供 latent space；理論上可把 AR-12B 視為 latent dynamics model。但官方未鋪這條路；社群實驗少。
