<!-- ontology-5axis output=pixel-video injection=data-only control=action|image-init|camera temporal=streaming-cache domain=robotics -->
...
- **Autoregressive latent diffusion**：在 latent space 一幀一幀往前 sample；不是一次 clip-parallel 整段 clip。
- **Causal-mask transformer dynamics**：跟 LLM 同 family；KV cache 讓滾動 inference 可行（這是把它放在 `temporal=streaming-cache` 而不是 `latent-rollout` 的關鍵）。
...
| Injection | `data-only` | 純靠大型 video dataset 隱式學物理；無 PDE/contact loss |
| Control | `action\|image-init` | 鍵鼠 action token + 起始幀 |
| Temporal | `streaming-cache` | causal-mask transformer + KV cache 滾動推 |
...
| **[Cosmos-Predict](../foundation-physics-models/cosmos-wfm.md)-action** (NVIDIA, 2025) | output=`pixel-video` · injection=`data-only` · temporal=`hierarchical` | Cosmos hierarchical 換 long-horizon，Genie 2 用 KV cache 換 interactivity |
...
- **Mechanism**：autoregressive loop + data-only → 每幀微小機率偏差 = 下一幀的訓練分布 OOD
