...
TOP_LEVEL_ORDER = ["ONBOARDING", "README", "AGENTS", "CONTRIBUTING", "MAINTAINER"]

# Files excluded entirely (exact stem in EXCLUDE_PATTERNS, or AUDIT_* prefix).
# - AUDIT_*: dated status / audit dumps (AUDIT_2026-05-25.md etc.)
...
def is_excluded_stem(stem: str) -> bool:
    return stem in EXCLUDE_PATTERNS or stem.startswith("AUDIT_")
...
def collect_md(d: Path) -> list[Path]:
    ...
    """
    files = [p for p in d.glob("*.md") if not is_excluded_stem(p.stem)]
    files += [p for p in d.glob("*.mdx") if not is_excluded_stem(p.stem)]
...
def main() -> int:
    ...
    docs = {
        ...
        "theme": "mint",
        "name": "Physics-Controllable Generation Handbook",
        "description": "Physics-as-conditioning world models — video / latent / 3D / simulator / surrogate · 5-axis ontology v2 (Injection ★ USP) · 照見三冊之三 (Generation side)",
        "colors": {
            "primary": "#7C3AED",
            "light": "#A78BFA",
            "dark": "#5B21B6",
        ...
        "navbar": {
            "links": [
                {"label": "GitHub", "href": "https://github.com/sou350121/Physics-Controllable-Generation-Handbook"},
                {"label": "VLA-Handbook", "href": "https://github.com/sou350121/VLA-Handbook"},
                {"label": "Spatial-Handbook", "href": "https://github.com/sou350121/Spatial-Intelligence-Handbook"},
            ]
        },
        "footer": {
            "socials": {
                "github": "https://github.com/sou350121/Physics-Controllable-Generation-Handbook",
            }
        },
    }
    ...
    def in_nav_scope(p: Path) -> bool:
        if ".git" in p.parts or is_excluded_stem(p.stem):
            return False
